...
1. bldgフォルダ内の建築物データからズームレベル23の空間IDを生成する。その際、空洞部の空間ID生成とズームレベルの最適化処理も合わせて実行する。
2. 生成された空間IDから空間IDが付与されたCityGML（建築物）を生成する。その際、CityGMLファイルへの空間ID直接付与とCSVファイルへの相対パス記録の両者を実行する。
3. urfフォルダ内の用途地域データからズームレベル20の空間ID（2次元）を生成する。
4. 生成された空間IDから空間IDが付与されたCityGML（用途地域）を生成する。その際、CityGMLファイルへの空間ID直接付与とCSVファイルへの相対パス記録の両者を実行する。
5. 2次元の空間IDが付与されたCityGMLファイルから空間IDを抽出し、標高-10mから100mの範囲を含む3次元の空間IDを生成する。
